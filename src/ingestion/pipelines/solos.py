...
from src.validation.pipelines.solos import validate_dataframe
...
# CloudWatch/SNS
LOG_GROUP_NAME = os.getenv("LOG_GROUP_NAME_SOLOS", "/ingestao/raw")
...
def validar_ingestao(stats: Dict[str, int]) -> Dict[str, Any]:
    """Valida ingestão de solos - wrapper para a função base"""
    return _validar_ingestao_base(stats)


def enviar_metricas(resultado: Dict[str, Any], observability) -> None:
    """Envia métricas para CloudWatch - wrapper customizado para solos"""
    import logging
    ...
    observability.enviar_metricas(
        ...
        dimensoes={'Camada': 'Raw', 'Fonte': 'Solos'}
    )
