...
Ingestão de dados de clima do INMET:
...
def ingerir_clima(**context):
    ...
    context['ti'].xcom_push(key='stats_ingestao_clima', value=stats)
    return stats


def validar_ingestao_clima(**context):
    """Valida ingestão e salva resultado no XCom"""
    ti = context['ti']
    stats = ti.xcom_pull(key='stats_ingestao_clima', task_ids='ingest_clima')
