...
Ingestão de dados de agricultura do INMET:
- Download de dados agrícolas
...
from src.ingestion.pipelines.agricultura import (
    ingest_data,
    validar_ingestao,
    enviar_metricas
)
...
def ingerir_agricultura(**context):
    ...
    context['ti'].xcom_push(key='stats_ingestao_agricultura', value=stats)
    return stats


def validar_ingestao_agricultura(**context):
    """Valida ingestão e salva resultado no XCom"""
    ti = context['ti']
    stats = ti.xcom_pull(key='stats_ingestao_agricultura', task_ids='ingest_agricultura')
